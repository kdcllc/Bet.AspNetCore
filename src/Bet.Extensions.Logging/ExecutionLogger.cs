        ...
        public async Task<T> ExecuteAndLogAsync<T>(Func<Task<T>> action, string methodName)
        {
            if (_logger.IsEnabled(LogLevel.Debug))
            {
                ...
                try
                {
                    ...
                    _logger.LogDebug("Method Name: {methodName}; Elapsed: {ElapsedMilliseconds} ms;", methodName, sw.GetElapsedTime());
                ...
                catch (Exception ex)
                {
                    ...
                    _logger.LogError(exception, "Method Name: {methodName}; Elapsed: {ElapsedMilliseconds} ms", methodName, sw.GetElapsedTime());
                    throw;
                }
            }
